    ...
    methods
        ...
        function assign_setA(node,num)
            if node.is_leaf
                ...
                assign_setA(node.Left,num);
                assign_setA(node.Right,num);
                node.sumChar = node.Left.sumChar + node.Right.sumChar;
                if node.sumChar>=num
                    node.setA = 1;
                else
                    node.setA = 0;
