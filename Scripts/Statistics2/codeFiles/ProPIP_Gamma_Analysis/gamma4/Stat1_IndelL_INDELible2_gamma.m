...
addpath(strcat('/Users/pouloeld/Documents/Statistics2/INDELible/ProPIP/Gamma','/',baseType4))
...
Inf_I_Path4= strcat('/Users/pouloeld/Documents/Statistics2/INDELible/ProPIP/Gamma','/',baseType4); 
Inf_I_files4= dir(fullfile(Inf_I_Path4,'*.fasta'));
Inf_I_Names4= {Inf_I_files4.name};
tk_i4= regexp(Inf_I_Names4,'((?<=ProPIPgamma4inferredMSA_I_)\d*)','match');
...
for namesI4=1:nIter_I4
    ...
    Inf_I_countMSA4= [];
    for nTaxa=1:8
        for seqL=1:length(Inf_I_MSA4(nTaxa).Sequence)
            if Inf_I_MSA4(nTaxa).Sequence(seqL) == '-'
                count= count+1;
            elseif Inf_I_MSA4(nTaxa).Sequence(seqL) ~= '-'
                if count ~= 0
                    Inf_I_count4= [Inf_I_count4,count];
                    Inf_I_countMSA4= [Inf_I_countMSA4,count];
%                     if count>29
%                         maxMSA4=namesI4;
%                     end
                    count=0;
...
%     if namesI4==41
%         MSA_stats{1}= [length(Inf_I_countMSA4),max(Inf_I_countMSA4),mean(Inf_I_countMSA4),median(Inf_I_countMSA4),std(Inf_I_countMSA4)];          
%     end
%     
%     disp(namesI4)
%     disp(Inf_I_countMSA4)
%     
%     figure,
%     histogram(Inf_I_countMSA4,'Normalization','count')
%     title(strcat('ProPIP+Gamma-n=4,a=0.1,-Inferred-MSA',int2str(namesI4)))
%     xlabel('Indel length')
%     ylabel('frequency')   
    ...
    figure,
    histogram(Inf_I_count4,'Normalization','count')
    title(strcat('ProPIP+Gamma-n=4,a=0.1,-Inferred-MSA',int2str(namesI4)))
    xlabel('Indel length')
    ylabel('frequency') 
...
stat4{1}= [length(Inf_I_count4),max(Inf_I_count4),mean(Inf_I_count4),median(Inf_I_count4),std(Inf_I_count4)];
