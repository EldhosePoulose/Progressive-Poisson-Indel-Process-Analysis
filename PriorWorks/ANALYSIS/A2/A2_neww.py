...
#d0 = "D:/ZHAW/1_SEMESTER1/TRACK 2/CLUSTER/PROGRESS/STEP1_TRUE_MSA_PIPJAVA/execs/"+str(i)+".exec/output/"
Tmsa_len0 = []
Tmsa_len1 = []
Tmsa_len2 = []
Tmsa_len3 = []
...
ax2.set_ylim(925,1150)

fig.suptitle('A2: Difference between the length of MSAs')
plt.show()'''
...
#yerr = df['True_std'],df['Inf_std'] #or
##yerr = df[['True_std','Inf_std']].values.T
...
diff = pd.DataFrame({'Inf. MSA len - True MSA len':(Inf_MSA_len-True_MSA_len).mean()})
std = (Inf_MSA_len-True_MSA_len).std()
diff.plot.bar(title = 'Bias in terms of MSA length',yerr = std ,rot = 0,grid = True,capsize = 4)
plt.ylabel('Difference between the length of MSAs')
plt.show()
